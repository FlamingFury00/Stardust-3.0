...
Goal-line saves are arrival-time driven rather than distance-threshold driven: the controller computes the flat speed needed to cover the remaining mouth displacement before the predicted crossing, preserves terminal mobility when the required arrival speed is high, and only permits a dodge when enough time remains to recover from the committed motion. Cars grounded behind their own goal line must first exit through the mouth before the normal loose-ball attack planner can reclaim control.

Shot selection still prefers lower-cost ground/jump options over expensive aerials when comparable contacts exist. The shot-ranking speed heuristic now uses the same initial contact-velocity geometry as the GroundShot/JumpShot solvers; it does **not** use the deliberately offset pre-contact car target as a velocity proxy. The estimate remains a ranking heuristic, not a full car-ball collision simulator. Emergency clears use the **own** goal as an exclusion target. Expensive searches remain separated from per-frame control, but actual p95/p99 tick latency must still be measured on the target hardware with multiple bots.
