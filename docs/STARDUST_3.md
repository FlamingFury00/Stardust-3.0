...
Goal-line saves are arrival-time driven rather than distance-threshold driven: the controller computes the flat speed needed to cover the remaining mouth displacement before the predicted crossing, stays in travel mode down to the true arrival radius when the crossing clock still requires speed, and only starts an elevated jump after reaching the goal-plane depth. Once a car already owns the correct crossing lane, that save is sticky: speculative intercept/staging actions cannot pull it away unless the ball is genuinely point-blank or the crossing moves materially. Before an active close-contact save is abandoned for a final-line sprint, the supervisor now applies an intentionally optimistic straight-line reach bound; if even that bound cannot reach the guard point in time, the existing contact save keeps ownership. Threat staging and EmergencyClear pre-contact waypoints are constrained to remain field-side of the own goal line, so a behind-ball offset cannot walk the car into its own net as the ball reaches the mouth. DefensiveDrive reverse mode is deliberately local: it is retained for a short mostly-behind correction but released as soon as a moving target becomes a long or substantially lateral route, allowing the existing turn/half-flip recovery logic to face the play again. Close EmergencyClear blocks prioritize contact over clear power: low balls drive almost through the predicted ball center rather than circling to a full shooting offset, mid-height balls stay steerable on a single jump, high balls may use a neutral second jump for vertical reach, and a directional dodge is armed only once distance, vertical separation, and nose alignment all indicate real contact range. Cars grounded behind their own goal line must first exit through the mouth before the normal loose-ball attack planner can reclaim control.
